...
function makeTable(data) {
	...
	if(data.length == 0)
		strHtml = "<tr><td colspan=\"2\">결과가 존재하지 않습니다.</td></tr>";
	
	for(var i=0; i < data.length; i++) {
		
		strHtml += "<tr id=\"addrTr\" onclick=\"addrAddDiv(this)\" data-dismiss=\"modal\">       "
...
//주소 선택하면 우편번호랑 주소 창에 선택한 주소 띄우기
function addrAddDiv(obj) {
	console.log(obj);
	var codeZip = $(obj).children().eq(0).text();
	var addr = $(obj).children().eq(1).text();
	$("#memZip").val(codeZip);
	$("#memAdd1").val(addr);
	
}

//[번호 검색]클릭하면 테이블 초기화 해주기
function resetTb() {
	$("#addrTb tbody").empty();
	$("#dong").val("");
	var strHtml = "<tr><td colspan=\"2\">검색버튼을 클릭하세요.</td></tr>";
	$("#addrTb tbody").html(strHtml);
}